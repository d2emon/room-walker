...
willwork:bprintf("You cast the summoning......\n");
    ...
    x = getItem(a).locationId;
    if(!getItem(a).isLicatedAt) x=ploc(x);
    sprintf(ms,"\001p%s\001 has summoned the %s\n",globme,oname(a));
    sendsys(globme,globme,-10000,x,ms);
    bprintf("The %s flies into your hand ,was ",oname(a));
    desrm(getItem(a).locationId,getItem(a).carryFlag);
    getItem(a).setCarriedBy(mynum);
    }
 ...
 ressurcom()
    ...
    if(!getItem(a).isDestroyed)
       {
       bprintf("That already exists\n");
       return;
       }
    getItem(a).create();
    getItem(a).setLocatedAt(curch);
