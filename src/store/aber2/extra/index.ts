...
if(brkword()!= -1)
{
 ...
 stacom()
    ...
    bprintf("\nItem        :%s",oname(a));
if(getItem(a).isContained) bprintf(       "\nContained in:%s",oname(getItem(a).locationId));
else
{
    if(!getItem(a).isLoactedAt)bprintf("\nHeld By     :%s",pname(getItem(a).locationId));
    else
       {bprintf("\nPosition    :");
       showname(getItem(a).locationId);
}
       }
    bprintf("\nState       :%d", state(a));
    bprintf("\nCarr_Flag   :%d", getItem(a).carryFlag);
    bprintf("\nSpare       :%d", !getItem(a).isDestroyed);
 ...
 examcom()
    ...
    switch(a)
       {
       case 144:
          if(getItem(144).damage==0)
             {
            getItem(144).damage = 1;
             bprintf("You take a scroll from the tube.\n");
            getItem(145).create();
            getItem(145).setCarriedBy(mynum);
             return;
             }
          break;
       case 145:
          ...
          return;
       case 101:
          if(getItem(101).damage==0)
             {
             bprintf("You take a key from one pocket\n");
             getItem(101).damage = 1;
             getItem(107).flags[DESTROYED] = false;
             getItem(107).setCarriedBy(mynum);
             return;
             }
          break;
       case 7:
...
if(my_lev>9999) bprintf("[%3d]",cha);
    ...
    if((my_lev<10)&& getItem(cha).isDestroyed) bprintf("Nowhere\n");
    else
       desrm(getItem(cha).locationId, getItem(cha).carryFlag);
    }
 cha++;
 }
 cha=fpbn(wordbuf);
