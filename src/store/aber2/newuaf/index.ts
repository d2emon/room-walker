...
l1:	i=(FILE *)personactl(name,&x,PCTL_FIND);
	...
	if(strcmp(x.p_name,name)) {
           return await dispatch(stopWithMessage('Panic: Invalid Persona Delete'));
    }
	strcpy(x.p_name,"");
	x.p_level= -1;
...
char *perm;
{
	...
	if(i==NULL)
	{
           return await dispatch(stopWithMessage('Cannot access UAF\n'));
	}
	return(i);
}
...
initme()
{
	...
	if(errno!=0){
           return await dispatch(stopWithMessage('Panic: Timeout event on user file\n'));
    }
	x.p_score=0;
