...
/*
 ...
 if(getItem(c).isDestroyed) bprintf("(");
          bprintf("%s",oname(c));
 ...
 {
 extern long my_lev;
 if(!getItem(o1).isContainedIn) return(0)
 ;
 if(getItem(o1).locationId!=o2) return(0);
 if((my_lev<10) && getItem(o1).isDestroyed)return(0);
 return(1);
 }
...
/*
 ...
 if((a==112)&&(des_inf==-1))
 {
 if(getItem(113).isDestroyed)) a=113;
 else if(getItem(114).isDestroyed) a=114;
 if((a==113)||(a==114)) {
   getItem(a).create();
 ...
 if((a==32)&&(state(a)==1)&&(ptothlp(mynum)==-1))
 {
     ...
     getItem(a).setItemIsCarriedBy(mynum);
 ...
 if(curch==-1081) 
 {
  ...
  ishere(item)
     ...
     long a;
 extern long my_lev;
     if((my_lev<10)&&getItem(item).isDestroyed)return(0);
     if(getItem(item).isWearingBy) return(0);
     if(getItem(item).locationId!=curch) return(0);
     return(1);
     }
 ...
 extern long my_lev;
     if((my_lev<10)&&getItem(item).isDestroyed)return(0);
     if((!getItem(item).isWearingBy)&&(!getItem(item).isCarriedBy)) return(0);
     if(getItem(item).locationId!=user) return(0);
     return(1);
     }
 ...
 return;
   ...
   getItem(a).setItemIsLocatedAt(curch);
    bprintf("OK..\n");
    ...
    bprintf("It disappears down into the bottomless pit.....\n");
     ...
     calibme();
getItem(a).setItemIsLocatedAt(-6);
     }
  lisobs()
     {
     lojal2(1);
        ...
        if((ishere(a))&&(oflannel(a)==n))
           {
               ...
               if(!!strlen(olongt(a,state(a)))) */
/*
OLONGT NOTE TO BE ADDED */
/*

         {
             if(getItem(a).isDestroyed) bprintf("--");
  ...
  dumpstuff(n,loc)
     ...
     while(b<NOBS)
        ...
        if(iscarrby(b,n))
           ...
           {
         getItem(b).setItemIsLocatedAt(loc);
           }
        b++;
        }
     }
